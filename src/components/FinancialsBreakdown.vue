<template>
        ...
        <table class="border-y-2 border-l-2 border-r-4 border-r-neutral-400 border-navy-950">
            ...
            <tbody>
                <tr v-for="breakdown in breakdowns[selectedFinancials]">
                    <td
                        :class="[
                            imptBreakdown.includes(breakdown)
                                ? 'font-semibold px-2'
                                : 'pl-10 pr-2',
    ...
    <div class="col-span-2 overflow-x-auto" v-if="loaded">
        <div class="max-w-full overflow-x-auto">
            <table class="table-fixed border-y-2 border-r-2 border-l-0 border-navy-950 text-center w-full">
                <thead>
                    <tr class="font-bold">
                        <td
                            class="sep"
                            v-for="formattedDate of formattedDates"
                            :key="formattedDate"
                            style="width: 200px;" 
                        >
                            {{ formattedDate }}
                ...
                <tbody>
                    <tr
                        v-for="(val, breakdown) in breakdowns[
                            selectedFinancials
                        ]"
                        :key="breakdown"
                    >
...
export default {
    ...
    methods: {
        ...
        loadFinancials() {
            ...
            axios
                .get(
                    `http://localhost:5000/stockprice/${this.selectedFinancials}/${this.stock}`
                )
                .then((response) => {
                    ...
                    for (let i = 0; i < 4; i++) {
                        let fiscalDate =
                            viewResponse[i].fiscalDateEnding.split("-");
                        ...
                        if (this.selectedFinancials == "incomestatement") {
                            this.viewReports[i].operatingExpenses =
                                ...
                                parseInt(
                                    viewResponse[i].researchAndDevelopment
                                );
                        }
                    }
